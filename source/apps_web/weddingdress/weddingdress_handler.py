...
from conf.work_conf import _QUERY_PRICE, PAGE_SIZE, _WEDDING_DRESS_CATEGORY, _WEDDING_DRESS_COLOR, _WEDDING_DRESS_STYLE
...
class WeddingDressSeriesHandler(BaseApiHandler, WebCacheHandler):
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            essence = self.get_essence(page_size=4, series_service=WeddingDressSeriesService())

            self.echo('view/weddingdress/series_list.html',{
...
class WeddingDressSeriesDetailHandler(BaseApiHandler, WebCacheHandler):
    def get(self, series_id):
        try:
            series_service, company_service = WeddingDressSeriesService(), CompanyServices()
            ...
            self.echo('view/weddingdress/series_detail.html',{
                'series':series,
...
class WeddingDressMerchantHandler(BaseApiHandler, WebCacheHandler):
    def get(self):
        self.get_paras_dict()
        ...
        try:
            query = company_service.get_companys_by_role('merchant_weddingdress', **self.qdict)     #婚纱礼服role_code: merchant_weddingdress
            ...
            self.echo('view/weddingdress/merchant_list.html',{
    ...
    def get_count(self,type,merchant_id):
        ...
        '''
        if type=='product':
            count = self.get_product_count(merchant_id, WeddingDressWorkService(self.db))
        elif type=='package':
            count = self.get_package_count(merchant_id, WeddingDressSeriesService(self.db))
        return count
...
class WeddingDressMerchantDetailHandler(BaseApiHandler, WebCacheHandler):
    def get(self, merchant_id):
        try:
            company_service, series_service = CompanyServices(self.db), WeddingDressSeriesService(self.db)
            ...
            self.echo('view/weddingdress/merchant_detail.html',{
                'company':company,
                'order_gift':order_gift,
                'company_gift':company_gift,
                'works':works,
...
class WeddingDressMerchantAllSeriesHandler(BaseApiHandler, WebCacheHandler):
    pass


class WeddingDressWorkHandler(BaseApiHandler, WebCacheHandler):
    def get(self):
        self.get_paras_dict()
        ...
        try:
            ...
            self.echo('view/weddingdress/work_list.html',{
...
class WeddingDressWorkDetailHandler(BaseApiHandler, WebCacheHandler):
    def get(self, work_id):
        try:
            work_service, company_service = WeddingDressWorkService(self.db), CompanyServices(self.db)
            ...
            order_gift = company_service.get_gift(work.Fmerchant_id,2).scalar()       #优惠
            #1.商户订单,2.套系订单 3.作品订单
            order_url = '/order/create/3/'+str(work_id)+'/'+str(company.Fuser_id)

            category = _WEDDING_DRESS_CATEGORY.get(work.Fcategory) if _WEDDING_DRESS_CATEGORY.get(work.Fcategory) else work.Fcategory
            color = _WEDDING_DRESS_COLOR.get(work.Fcolor) if _WEDDING_DRESS_COLOR.get(work.Fcolor) else work.Fcolor
            style = _WEDDING_DRESS_STYLE.get(work.Fstyle) if _WEDDING_DRESS_STYLE.get(work.Fstyle) else work.Fstyle

            self.echo('view/weddingdress/work_detail.html',{
                'work':work,
                'images':images,
...
class WeddingDressMerchantAllWorkHandler(BaseApiHandler, WebCacheHandler):
    pass
